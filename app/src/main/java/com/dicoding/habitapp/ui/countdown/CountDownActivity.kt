    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (habit != null){
            ...
            findViewById<Button>(R.id.btn_start).setOnClickListener {
                ...
                val timeInitial = viewModel.getInitialTime()
                if (timeInitial != null){
                    val myData = workDataOf(
                        HABIT_ID to habit.id,
                        HABIT_TITLE to habit.title
                    )
                    val workManagerNotification : WorkRequest =
                        OneTimeWorkRequestBuilder<NotificationWorker>()
                            .setInputData(myData)
                            .setInitialDelay(timeInitial, TimeUnit.MILLISECONDS)
                            .build()
                    WorkManager
                        .getInstance(this)
                        .enqueue(workManagerNotification)
                }
            }
