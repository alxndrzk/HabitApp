    ...
    override fun doWork(): Result {
        ...
        if (shouldNotify) {
            if (habitTitle != null){
                ...
                val intentPending = TaskStackBuilder.create(applicationContext).run {
                    addNextIntentWithParentStack(intent)
                    getPendingIntent(0, PendingIntent.FLAG_IMMUTABLE)
                }
                val managerNotification = applicationContext
                    .getSystemService(Context.NOTIFICATION_SERVICE) as NotificationManager
                val builderNotification = NotificationCompat.Builder(applicationContext, NOTIFICATION_CHANNEL_ID)
                ...
                managerNotification.notify(100, notification)
            }
        }
