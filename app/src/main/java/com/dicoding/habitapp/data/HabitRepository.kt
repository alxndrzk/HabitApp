...
class HabitRepository(private val habitDao: HabitDao, private val executor: ExecutorService) {
    ...
    fun getHabits(sortType: HabitSortType): LiveData<PagedList<Habit>> {
        val querySorted = SortUtils.getSortedQuery(sortType)
        val configPagedList = PagedList.Config.Builder()
            .setEnablePlaceholders(false)
            .setPageSize(20)
            .build()
        return LivePagedListBuilder(habitDao.getHabits(querySorted), configPagedList).build()
    }
